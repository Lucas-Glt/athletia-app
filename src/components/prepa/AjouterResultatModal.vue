  ...
  <div class="modal-overlay" @click.self="$emit('fermer')">
    <div class="modal modal-large">
      ...
      <div class="modal-body">
        ...
        <div class="bloc" v-if="test.categorie === 'musculation'">
        ...
        <div class="bloc champs-resultat">
          ...
          <template v-if="!enMode1RM">
            <div class="field">
              <label>Résultat ({{ test.unite }})</label>
...
import { REPS_MAX_ESTIMATION_1RM, estimer1RM } from '../../data/catalogueTests'
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const enMode1RM = computed(() => modeCalcul1RM.value && props.test.categorie === 'musculation')
    ...
    const peutEnregistrer = computed(() => {
      if (!athleteChoisi.value || !date.value) return false
    ...
    const enregistrer = async () => {
      ...
      try {
        ...
        const notesFinal = enMode1RM.value
          ...
          : notes.value
        const data = await api.post(`/tests/${props.test.id}/resultats`, {
        ...
        erreur.value = "Erreur lors de l'enregistrement du résultat."
      } finally {
        enCours.value = false
      }
    }
...
.mode-1rm-toggle { display: flex; gap: 6px; flex-wrap: wrap; }
.chip-filtre {
  display: inline-flex;
  align-items: center;
  gap: 6px;
